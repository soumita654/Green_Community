  ...
  useEffect(() => {
    const handleAuthCallback = async () => {
      ...
      if (data.session) {
        ...
        if (provider === 'google' || provider === 'github') {
          toast.success(`Welcome! Successfully signed in with ${provider === 'google' ? 'Google' : 'GitHub'} 🌱`);
        }
      }
    };
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === 'SIGNED_IN' && session) {
        ...
        if (provider === 'google' || provider === 'github') {
          toast.success(`Welcome back! Signed in with ${provider === 'google' ? 'Google' : 'GitHub'} 🌱`);
        }
      }
    });
